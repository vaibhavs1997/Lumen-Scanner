...
function main(argv) {
  ...
  // Windows cannot execute the npm-generated `vite.cmd` shim through
  // child_process without a shell. Invoke Vite's JS entry directly instead;
  // this is also deterministic when PATH does not include node_modules/.bin.
  const isVite = command === "vite";
  const executable = isVite ? process.execPath : command;
  const childArgs = isVite
